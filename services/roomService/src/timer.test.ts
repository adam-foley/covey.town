...
describe('CoveyTownController', () => {
  let timer : Timer
  let delay : number
  let createTimeTime : number  
  beforeEach(() => {
    jest.useFakeTimers();
    delay = 60000
    timer = new Timer(() => { return true; }, delay);
    createTimeTime = new Date().getTime();
  });
  afterEach(() =>{
    timer.clearTimer();
  });
  it('should get elapsed seconds for the current timer when time has elapsed', () => {
    ...
    Date.now = jest.fn().mockImplementation(()=> createTimeTime + 35000);
  ...
  it('should get elapsed seconds for the current timer when time has not elapsed', () => {
    ...
    Date.now = jest.fn().mockImplementation(()=> createTimeTime);
